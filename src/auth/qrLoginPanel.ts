...
export async function showQrLoginPanel(
    ...
    const updateStatus = (status: QrLoginStatus) => {
        lastStatus = status;
        if (webviewReady) void panel.webview.postMessage({ type: "status", ...status });
    };
