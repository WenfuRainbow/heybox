...
侧边栏工具栏提供登录和退出登录按钮：
- **登录** — 弹出登录方式选择：
  - **浏览器登录（推荐）** — 插件用系统已装的 Edge/Chrome 打开小黑盒登录页，手动完成登录（密码/短信/扫码均可）后自动获取 Cookie，无需手动复制。登录后插件会自动加载首页并打开一个真实帖子页面；若出现人机验证（滑块/点选）请在窗口内完成，确认帖子能正常打开后点击弹窗按钮即完成登录
  - **手动粘贴 Cookie** — 粘贴从浏览器复制的 Cookie 即可登录
- **退出登录** — 清除已保存的 Cookie
...
2. 点击侧边栏工具栏的登录按钮，选择「浏览器登录」自动获取 Cookie
3. 或者：打开浏览器访问 [xiaoheihe.cn](https://www.xiaoheihe.cn) 并登录，按 `F12` 打开开发者工具 → Network → 复制 Cookie，点击登录按钮选择「手动粘贴 Cookie」粘贴
