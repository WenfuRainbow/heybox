...
- **论坛就在侧边栏**：横向切换推荐、社区、收藏和消息；卡片列表、热搜、历史、分页与帖子详情也都不用跳出 VS Code。
...
- **推荐 / 社区 / 收藏 / 消息**：默认使用横向导航和富内容卡片；280–420px 的窄侧栏会自动压缩布局。隐身或简约模式继续使用原生 TreeView。
- **搜索与历史**：搜索框支持搜索全站帖子，“热搜”展示搜索首页建议，“历史”可重新打开最近浏览的 50 个帖子。
- **收藏同步**：展示账号中的实际收藏夹并可横向切换；右键原生树帖子仍可收藏或取消收藏。
- **分页兜底**：推荐和收藏支持滚动触底加载，同时始终保留“加载更多”按钮。
- **消息中心**：按需查看评论与回复、获赞、关注、@我、官方消息和游戏优惠；互动消息可直接跳转到对应帖子。

### 看帖与评论

点击列表项即可打开帖子详情。正文图片按原文位置呈现，评论支持楼层、嵌套回复、默认/最热/最新排序、按需展开和“加载全部评论”；详情底部还会展示相关推荐。点击卡片作者可只读浏览该作者的帖子。
